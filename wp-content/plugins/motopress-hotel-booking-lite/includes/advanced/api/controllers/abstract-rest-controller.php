...
abstract class AbstractRestController extends WP_REST_Controller {
	...
	protected function add_additional_fields_schema( $schema ) {
		...
		$schema['properties'] = apply_filters( 'mphb_rest_' . $object_type . '_schema', $schema['properties'] );
	...
	protected function check_batch_limit( $items ) {
		$limit = apply_filters( 'mphb_rest_batch_items_limit', 100, $this->get_normalized_rest_base() );
		$total = 0;
		...
		if ( $total > $limit ) {
			return new WP_Error( 'mphb_rest_request_entity_too_large', sprintf( 'Unable to accept more than %s items for this request.', $limit ), array( 'status' => 413 ) );
		}
