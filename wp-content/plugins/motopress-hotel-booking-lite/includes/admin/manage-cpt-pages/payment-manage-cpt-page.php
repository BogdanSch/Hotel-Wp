	...
	public function renderColumns( $column, $postId ){
		...
		switch ( $column ) {
			...
			case 'mphb_status':
				...
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				?><span class="column-status-<?php echo esc_attr( $payment->getStatus() ); ?>"><?php echo mphb_get_status_label( $payment->getStatus() ); ?></span><?php
			...
			case 'mphb_amount':
				...
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo mphb_format_price( $payment->getAmount(), array(
	...
	public function extendPostsSearch( $where, $wp_query ){
		...
		if ( $this->isCurrentPage() && !empty( $wp_query->query['s'] ) ) {
			...
			if ( is_email( $search ) ) {
				...
				} else if ( get_post_type( $search ) === MPHB()->postTypes()->payment()->getPostType() ) {
