		...
		switch ( $column ) {
			...
			case 'check_in_out_date' :
				...
				if ( !is_null( $checkInDate ) && !is_null( $checkOutDate ) ) {
					...
					?><span class="mphb-booking-nights"><?php echo esc_html( sprintf( _n( '%s night', '%s nights', $nights, 'motopress-hotel-booking' ), $nights ) ); ?></span><?php
				}
                ...
                if ($booking->isImported()) {
                    ...
                    <?php else: ?>
                        <span aria-hidden="true"><?php 
							// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							echo static::EMPTY_VALUE_PLACEHOLDER; ?></span>
                    <?php endif; 
                }
				break;
			case 'price':
