...
class BookingsQuery
{
    ...
    protected function parseQuery($args)
    {
        ...
        // Build query args
        $queryArgs = array('orderby' => 'ID', 'order' => 'ASC');
        ...
        if (!empty($startDate)) {
            switch ($searchBy) {
                case 'reserved-rooms': $metaQuery[] = array('key' => 'mphb_check_in_date',  'value' => $startDate, 'compare' => '>='); break;
                case 'check-in':       $metaQuery[] = array('key' => 'mphb_check_in_date',  'value' => $startDate, 'compare' => '>='); break;
                case 'check-out':      $metaQuery[] = array('key' => 'mphb_check_out_date', 'value' => $startDate, 'compare' => '>='); break;
                case 'in-house':       $metaQuery[] = array('key' => 'mphb_check_out_date', 'value' => $startDate, 'compare' => '>='); break;
                case 'booking-date':   break;
                default: $metaQuery = apply_filters('mphb_export_bookings_start_date_meta_query', $metaQuery, $startDate, $searchBy);  break;
            }
        }

        if (!empty($endDate)) {
            switch ($searchBy) {
                case 'reserved-rooms': $metaQuery[] = array('key' => 'mphb_check_out_date', 'value' => $endDate, 'compare' => '<='); break;
                case 'check-in':       $metaQuery[] = array('key' => 'mphb_check_in_date',  'value' => $endDate, 'compare' => '<='); break;
                case 'check-out':      $metaQuery[] = array('key' => 'mphb_check_out_date', 'value' => $endDate, 'compare' => '<='); break;
                case 'in-house':       $metaQuery[] = array('key' => 'mphb_check_in_date',  'value' => $endDate, 'compare' => '<='); break;
                case 'booking-date':   break;
                default: $metaQuery = apply_filters('mphb_export_bookings_end_date_meta_query', $metaQuery, $endDate, $searchBy); break;
            }
        }
