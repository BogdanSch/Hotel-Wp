...
//			),
	...
	private function detectCustomFieldsDefaults(){
		...
		if ( empty( $_GET['mphb_defaults'] ) || ! is_array( $_GET['mphb_defaults'] ) ) return array();
	...
	public function getFieldGroups(){
		...
		$gatewaysList = array_map( function($gateway) {
			return $gateway->getTitle();
		}, MPHB()->gatewayManager()->getList() );

		$bookings		 = MPHB()->getBookingPersistence()->getPosts( array(
			'fields' => 'ids'
			)
		);
		...
		$paymentFields = array(
			Fields\FieldFactory::create( '_id', array(
				'type'		 => 'post-id',
