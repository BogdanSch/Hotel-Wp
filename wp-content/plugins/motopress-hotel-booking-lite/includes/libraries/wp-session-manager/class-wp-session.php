...
final class WP_Session extends Recursive_ArrayAccess implements \Iterator, \Countable {
	 ...
	 * Singleton instance.
	 *
	 * @var bool|\MPHB\Libraries\WP_SessionManager\WP_Session
	 */
	 ...
	 *
	 * @return bool|\MPHB\Libraries\WP_SessionManager\WP_Session
	 */
	 ...
	 *
	 * @uses apply_filters Calls `mphb_wp_session_expiration_variant` to get the max update window for session data.
	 * @uses apply_filters Calls `mphb_wp_session_expiration` to get the standard expiration time for sessions.
	 */
	protected function set_expiration(){
		$this->exp_variant	 = time() + (int) apply_filters( 'mphb_wp_session_expiration_variant', 24 * 60 );
		$this->expires		 = time() + (int) apply_filters( 'mphb_wp_session_expiration', 30 * 60 );
	}
	...
	protected function read_data(){
		$this->container = get_option( "_mphb_wp_session_{$this->session_id}", array() );
	...
	public function write_data(){
		$option_key = "_mphb_wp_session_{$this->session_id}";

		// Only write the collection to the DB if it's changed.
		if ( $this->dirty ) {
			if ( false === get_option( $option_key ) ) {
				add_option( "_mphb_wp_session_{$this->session_id}", $this->container, '', 'no' );
				add_option( "_mphb_wp_session_expires_{$this->session_id}", $this->expires, '', 'no' );
			} else {
				delete_option( "_mphb_wp_session_{$this->session_id}" );
				add_option( "_mphb_wp_session_{$this->session_id}", $this->container, '', 'no' );
			}
		}
	}
