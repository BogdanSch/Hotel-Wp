...
function mphb_tmpl_the_room_reservation_form( $roomTypeId = 0 ) {
	...
	?>
	<form method="<?php echo esc_attr( $formMethod ); ?>" action="<?php echo esc_url( $actionUrl ); ?>" class="mphb-booking-form <?php echo esc_attr($formDirectBookingClass); ?>" id="<?php echo esc_attr( 'booking-form-' . $typeId ); ?>">
		...
		<?php if ( !$isDirectBooking || $directBookingPricing == 'capacity' ) { ?>
			...
			<?php } else { ?>
				...
				<p class="mphb-adults-wrapper mphb-capacity-wrapper">
					...
					<select id="<?php echo esc_attr( 'mphb_adults-' . $uniqueSuffix ); ?>" name="mphb_adults">
						<?php 
							$maxAdultsCount = $isDirectBooking ? $roomType->getAdultsCapacity() : MPHB()->settings()->main()->getSearchMaxAdults();

							foreach ( range( MPHB()->settings()->main()->getMinAdults(), $maxAdultsCount ) as $value ) { ?>
			...
			<?php 
				$maxChildrenCount = $isDirectBooking ? $roomType->getChildrenCapacity() : MPHB()->settings()->main()->getSearchMaxChildren();
			...
			<?php } else { ?>
				...
				<p class="mphb-children-wrapper mphb-capacity-wrapper">
					...
					<select id="<?php echo esc_attr( 'mphb_children-' . $uniqueSuffix ); ?>" name="mphb_children">
						...
						<?php foreach ( range( 0, $maxChildrenCount ) as $value ) { ?>
    ...
    <table class="mphb-payments-table">
        ...
        <tfoot>
            <tr>
                ...
                <th><?php 
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo mphb_format_price($totalPaid); ?></th>
            </tr>
            <tr>
...
function renderTaxesAndFeesOutput( $taxesAndFees, $paragraph = true ) {
	if( $taxesAndFees->hasTaxesAndFees() ) {
		if( !$taxesAndFees->areTaxesAndFeesDefined() ) {
		...
		} else if( $taxesAndFees->hasExcludedTaxesAndFees() ) {
			...
			return $output;
		} else if( $taxesAndFees->hasIncludedTaxesAndFees() ) {
